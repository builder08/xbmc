                                ...
                                RegexCache* regexCache /*= nullptr*/)
{
  ...
  for (const auto &regexp : regexps)
  {
    ...
    {
      CRegExp* regexToInitilize = nullptr;
      if (regexCache)
      {
        ...
        if (!inserted)
          ...
          regexToInitilize = &(iter->second);
      }
      else
        // the local regex needs to be always (re)initialized
        regexToInitilize = &(*localRegex);

      if (!regexToInitilize->RegComp(regexp.c_str()))
      {
